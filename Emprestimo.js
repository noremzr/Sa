...
function Consultar(){
    u.clear().draw(false);
     ...
     var url = `/consultaEmprestimo/${codLeitor}/${codLivro}`;
          var xhr = new XMLHttpRequest();
          ...
          xhr.onreadystatechange = function(){
               if (xhr.readyState == 4 && xhr.status == 200){
                    ...
                    for (e in data){
                        ...
                        var dataDevolucao = ""
                        if (data[e]["dataDevolucao"] != null && data[e]["dataDevolucao"] != "" ){
                         dataDevolucao = new String(data[e]["dataDevolucao"]).substring(0,new String(data[e]["dataDevolucao"]).indexOf("T"))
                    }
                        u.row.add([
                              `<button type='button' class='btn btn-danger'  onclick="FinalizarEmprestimo('${data[e]["codEmprestimo"]}', '${data[e]["codLivro"]}','${data[e]["SituacaoEmprestimo"]}')"><i class="fa fa-plus" aria-hidden="true"></i></button>`,
                                                  data[e]["codEmprestimo"],
                                                  data[e]["codLivro"],
                                                  data[e]["titulo"],
                                                  data[e]["codLeitor"],
                                                  data[e]["nome"],
...
function FinalizarEmprestimo(codEmprestimo,codLivro,situacao){
    if (situacao != "Aberto"){
        alert("Empréstimo já finalizado");
    }
    else if(confirm("Quer Finalizar este empréstimo?")){
        var body = {
            codLivro:codLivro,
